...
public class PageEventRestController {
    ...
    public pageEvent publish(@PathVariable String topic, @PathVariable String name) {
        // Génère un événement page avec les bons paramètres
        pageEvent pageEvent = new pageEvent(
                name,                         // Correspond au champ 'name'
                Math.random() > 0.5 ? "U1" : "U2",  // Correspond au champ 'user'
                new Date(),                   // Correspond au champ 'date'
                new Random().nextLong(9000)   // Correspond au champ 'duration'
        );

        // Envoie l'événement au topic
        streamBridge.send(topic, pageEvent);

        return pageEvent;  // Retourne l'événement
    }
